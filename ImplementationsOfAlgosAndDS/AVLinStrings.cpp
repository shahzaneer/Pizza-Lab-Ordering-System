...
// two Strings are compared with each other on the basis of lexicographical differences
// it refers to the dictionary ordering.
// for example "ABBA" is to be compared with "ABBC" then from left to right unicode of each character
// will be measured. Now see "ABB" are common in both then 'A' < 'C' (49 < 51)
// therefore  "ABBA" < "ABBC"
...
int search(AVLNode *root, string key)
{
    if (root == NULL)
    {
        return 0;
...
string minValueAVL(AVLNode *root)
{
...
string maxValueAVL(AVLNode *root)
{
...
AVLNode *insertion(string value, AVLNode *root)
{
...
int main()
{
    ...
    root = insertion("Shahzaneer Ahmed", root);
    root = insertion("Pakistan", root);
    root = insertion("Computer Science", root);
    root = insertion("Data Science", root);
    root = insertion("Mobile App Development", root);
    root = insertion("System Design", root);
